...
The KeyPackage feature is designed to securely package and transfer key data,
including public and private keys, between devices. This allows users to keep
their OpenPGP identities available across platforms.

Starting with version 2.1.9, KeyPackage security was significantly improved by
moving away from the older AES-ECB based format. Starting with v2.2.0,
the internal cryptographic helpers used by KeyPackage handling have migrated
from OpenSSL to **libsodium**, and the internal package format has changed again.

<Aside type="caution">
  KeyPackages created with v2.1.9 and later are not compatible with packages
  created by versions earlier than v2.1.9.

KeyPackages created with v2.2.0 and later may also be incompatible with
KeyPackages created by v2.1.9 to v2.1.x, because the internal cryptographic
format changed as part of the migration from OpenSSL-based helpers to
libsodium-based helpers.
...
KeyPackage data was migrated from the older AES-256-ECB based format to an
authenticated encryption design based on OpenSSL-backed AES-GCM. This improved
both confidentiality and integrity protection.

Earlier versions used AES-256-ECB, which did not provide integrity checking and
is no longer recommended for this purpose.

### Starting with v2.2.0

KeyPackage handling now uses GpgFrontend's libsodium-based internal
cryptographic helpers instead of OpenSSL-based helpers.

This change improves consistency with the rest of GpgFrontend's internal
application-managed cryptography, but it also changes the internal KeyPackage
format. As a result, KeyPackages should generally be created and imported with
the same major GpgFrontend series when possible.
