...
This is important because OpenPGP v6 Ed25519/X25519 keys are not encoded in the
same way as the older OpenPGP 25519 keys commonly generated by GnuPG. Existing
GnuPG workflows usually use the legacy OpenPGP 25519 formats, while OpenPGP v6
uses newer formats.

In GpgFrontend, this means:

- **rPGP can import and use 25519 keys generated by GnuPG**, including existing
  legacy Curve25519/Ed25519 OpenPGP keys.
- **rPGP-generated OpenPGP v6 25519 keys cannot currently be imported into
  GnuPG**. These keys should be treated as rPGP/OpenPGP v6 keys.
...
- If you want to test OpenPGP v6 key generation, use the rPGP engine, but do not
  expect those generated keys to work in GnuPG.

For most users, the practical rule is simple: **GnuPG-generated 25519 keys can be
used by rPGP, but rPGP-generated OpenPGP v6 25519 keys should be considered
rPGP-only for now.**
...
mechanism used for encryption-related workflows. In OpenPGP, it is typically
used in hybrid form, combined with classical elliptic-curve key exchange.

GpgFrontend may expose hybrid KEM choices depending on the active engine and
engine version. These combinations are intended to combine a classical ECDH
component with a post-quantum KEM component.
...
classic RSA or Curve25519 keys. Users who need maximum compatibility should use
stable GnuPG-compatible algorithms instead.
