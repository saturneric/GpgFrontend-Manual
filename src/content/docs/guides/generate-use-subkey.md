...
Subkeys in GpgFrontend offer more algorithm choices than primary keys because
they are usually created for specialized roles. A primary key is mainly used to
represent identity, certify user IDs, and establish trust, while subkeys are
commonly dedicated to encryption, signing, or authentication.

This task-specific design allows subkeys to use algorithms that are not suitable
as primary keys. For example, ECDH-based algorithms such as Curve25519, X448,
NIST curves, or Brainpool curves are designed for encryption/key-exchange
workflows and therefore appear as subkey options rather than primary-key
algorithms.

Available algorithms depend on the selected OpenPGP engine, engine version, and
key format. GnuPG remains the recommended default engine for maximum
interoperability, while rPGP provides experimental support for OpenPGP v6 and
some post-quantum algorithm choices.

For more details, see [Comparison of Cryptographic Algorithms](/extra/algorithms-comparison).

**Subkey Algorithm Availability:**
...
:::caution[OpenPGP v6 and rPGP Compatibility]
...
Kyber / ML-KEM hybrid encryption is not rPGP-only. It may also be available with
GnuPG 2.5+ depending on the selected key database engine, engine version, and
OpenPGP version. In the current rPGP engine, only selected hybrid KEM pairings
are supported, such as **Kyber-768 + Curve25519** and **Kyber-1024 + X448**.
Other pairings may require GnuPG 2.5+ or may not be available in the selected
engine.
