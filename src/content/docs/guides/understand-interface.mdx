...
The panel offers two views, selectable with the segmented switch at the top:

- **Status**: A summary report of the operation, presented as an easy-to-read
  card view.
- **Details**: The raw status text exactly as reported by the engine.

Your choice is remembered across operations and sessions, so the panel keeps
showing the view you prefer.

### Panel Actions

- **Copy**: Copies the status text to the clipboard.
- **Save**: Saves the status text as a plain text file (UTF-8).
- **Magnify**: Opens the generated summary document in a larger view. This
  action is only available while the **Status** view is showing a generated
  document.
...
columns and codes(in the Type and Usage columns) may not be immediately obvious
for new users. This section explains the meaning of these fields and codes, so
you can confidently interpret key types, usage, and status in your daily
...
4. **Decrypt**: Decrypt the currently selected encrypted file(s) or text. No key
   selection is required, because gpg will automatically detect and use the
